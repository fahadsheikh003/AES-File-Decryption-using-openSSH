...
bool seal( const char * inFile, const char * outFile, const char * publicKeyFile, const char * symmetricCipher)
{
    ...
    // no padding was used in sample data
    ...
    size_t key_len_encrypted;
    if (EVP_PKEY_encrypt(public_ctx, NULL, &key_len_encrypted, key, key_len) <= 0) {
        cerr << "Failed to find the length of encrypted key" << endl;
        EVP_PKEY_CTX_free(public_ctx);
        return false;
    }

    // Allocate memory for the encrypted key
    unsigned char encrypted_key[key_len_encrypted];

    // Encrypt the key
    if (EVP_PKEY_encrypt(public_ctx, encrypted_key, &key_len_encrypted, key, key_len) <= 0) {
    ...
    fwrite(&key_len_encrypted, sizeof(int), 1, output);

    //write encrypted key in the output file
    fwrite(encrypted_key, sizeof(unsigned char), key_len_encrypted, output);
...
int main ( void )
{
    assert( seal("out.PNG", "1sealed.bin", "PublicKey.pem", "aes-128-cbc") );
    assert( seal("out.PNG", "2sealed.bin", "PublicKey.pem", "aes-192-cbc") );
    assert( seal("out.PNG", "3sealed.bin", "PublicKey.pem", "aes-256-cbc") );
